...
const Result = () => {
  ...
  const {genarateImage} = useContext(AppContext); 
  ...
  const onSubmitHandler = async (e) => {
    ...
    if (input) {
      const image = await genarateImage(input);
      if (image) {
  ...
  return (
    ...
    >
      <div>
        <div className='relative'>
          ...
          <span className={`absolute bottom-0 left-0 h-1 bg-red-500 ${loading ? 'w-full transition-all duration-[5s]' :'w-0'}`}/>
        </div>
    ...
    <div className='flex w-full max-w-xl bg-neutral-500 text-white text-sm p-0.5 mt-10 rounded-full'>
      ...
      <button type='submit' className='bg-pink-700 px-10 sm:px-16 py-3 rounded-full cursor-pointer'>Generate</button>
    </div>
    }
{isImageLoaded && 
